...
function renderCheckinList() {
  ...
  list.querySelectorAll('.present-btn').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      const name = btn.closest('.member-row').querySelector('span').textContent;
...
async function loadAnalytics() {
  ...
  tbody.innerHTML = '<tr><td colspan="5" class="hint">Loading&hellip;</td></tr>';

  await loadMembersOnce();

  // All Wednesday/Friday events ever logged -- this is the "core" schedule
  // that attendance % and the inactive flag are measured against. Sunday
  // Prayer (optional) and Special Saturdays (irregular) don't count toward
  // either, same as the "missed last 3" logic below.
  ...
  // All attendance rows with their event's date + type
  const { data: rows } = await sb.from('attendance')
    .select('member_id, event_id, events(date, type)');

  const byMember = {};
  (rows || []).forEach((r) => {
    if (!byMember[r.member_id]) {
      byMember[r.member_id] = { total: 0, coreAttended: 0, lastDate: null, hitLast3Core: false };
    }
    const stat = byMember[r.member_id];
    stat.total += 1;

    const d = r.events ? r.events.date : null;
    if (d && (!stat.lastDate || d > stat.lastDate)) stat.lastDate = d;

    if (coreEventIds.has(r.event_id)) stat.coreAttended += 1;
    if (last3CoreIds.has(r.event_id)) stat.hitLast3Core = true;
  });

  const enoughCoreHistory = totalCoreEvents >= 3;

  tbody.innerHTML = allMembers.map((m) => {
    const stats = byMember[m.id] || { total: 0, coreAttended: 0, lastDate: null, hitLast3Core: false };
    ...
    if (badges.length === 0) badges.push('<span class="badge badge-active">Active</span>');
