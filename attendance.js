...
async function loadAnalytics() {
  ...
  const { data: statsRows } = await sb.from('member_core_attendance')
    .select('member_id, total_attended, core_attended, last_attended_date, hit_last_3_core');

  const byMember = {};
  (statsRows || []).forEach((r) => {
    byMember[r.member_id] = {
      total: r.total_attended,
      coreAttended: r.core_attended,
      lastDate: r.last_attended_date,
      hitLast3Core: r.hit_last_3_core,
    };
  });
