async function render() {
  ...
  // ---------- Attendance today ----------
  // Only today's rows are fetched here, not all-time history -- this stays
  // small regardless of how much attendance history has built up.
  ...
  if (todaysEventIds.size > 0) {
    const { data: todaysAttendance } = await sb.from('attendance')
      .select('member_id, event_id')
      .in('event_id', [...todaysEventIds]);
    presentTodayCount = new Set((todaysAttendance || []).map((r) => r.member_id)).size;
  }
  document.getElementById('stat-today').textContent = presentTodayCount;

  // ---------- Inactive members ----------
  // Pulls one pre-aggregated row per member from the database (via the
  // member_core_attendance view) instead of every raw attendance row --
  // this is what keeps the page fast as the fellowship grows.
  ...
  const { data: statsRows } = await sb.from('member_core_attendance').select('member_id, core_attended');
  const statsByMember = {};
  (statsRows || []).forEach((r) => { statsByMember[r.member_id] = r.core_attended; });

  const withPct = groupMembers
    .map((m) => {
      const attended = statsByMember[m.id] || 0;
  ...
  if (!window.iAmAdmin) {
    ...
    list.innerHTML = top5.map((m) => `
      <div class="card" style="margin-bottom:0.5rem; display:flex; justify-content:space-between; align-items:center;">
        <span>${m.name}</span>
        <span>
          <span class="badge ${m.pct < 50 ? 'badge-inactive' : 'badge-active'}">${m.pct}%</span>
        </span>
      </div>
    `).join('');
    return;
  ...
  list.innerHTML = top5.map((m) => `
    <div class="card followup-action" data-member-id="${m.id}" style="margin-bottom:0.6rem; display:flex; flex-direction:column; gap:0.5rem;">
      <div style="display:flex; justify-content:space-between; align-items:center;">
        <span>${m.name}</span>
        <span class="badge badge-inactive">${m.pct}%</span>
      </div>
  ...
  list.querySelectorAll('.assign-btn').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      const name = wrapper.querySelector('span').textContent;
      const select = wrapper.querySelector('.leader-select');
      const leaderId = select.value;
      if (!leaderId) {
        showToast('Pick a leader first', 'error');
        return;
      }
      ...
      wrapper.querySelector('.followup-controls').innerHTML = '<span class="badge badge-pending">Assigned &#9989;</span>';
      showToast(`Follow-up assigned for ${name}`, 'success');
    });
  });
}
